...
- 재질·FURN1·권리 정보를 포함하는 결정론적 glTF 2.0 GLB
- OBJ·MTL·FURN1·3D metadata·LICENSE를 보존하는 ZIP 묶음
...
- MagicaVoxel `.vox` 내보내기와 255색·256셀 제한을 다루는 명시적 변환 정책
...
1. **glTF 2.0 `.glb` — 현재 제공**: Blender, 3ds Max, SketchUp 같은 일반 3D 도구를 잇는 기본 메시 교환 파일이다. 같은 색·재료의 맞닿은 복셀 면을 결정론적으로 합쳐 내부 면과 폴리곤 수를 줄이고, 표면색을 unlit base-color 재료로 보존한다. 축 방향·단위·바닥 기준점과 원본 FURN1은 노드 `extras`에 기록하되 외부 도구가 이를 보존한다고 가정하지 않는다.
2. **OBJ + MTL + metadata ZIP — 현재 제공**: 단순 형상과 색을 넓게 전달하는 보조 형식이다. GLB와 같은 표면 메시를 사용하고 FURN1·축/단위 metadata·LICENSE를 함께 묶는다. 텍스트 기반이라 계층, 애니메이션, 복잡한 재질 보존에는 적합하지 않다.
3. **MagicaVoxel `.vox` — 다음 후보**: 복셀 좌표를 가장 직접적으로 보존한다. 공개된 version 150 구조는 좌표 축마다 1바이트를 쓰고 팔레트 인덱스 1~255를 사용하므로, 축 길이 256 이상 또는 255색을 넘는 가구는 분할·색상 축소 정책을 사용자가 확인해야 한다. MagicaVoxel 프로그램 자체를 포함하거나 재배포하지 않고 공개 포맷 설명만 바탕으로 writer를 독립 구현한다.
